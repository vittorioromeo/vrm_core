...
namespace vrm::core::resource::impl
{
    ...
    template <typename TBehavior, typename TLockPolicy>
    shared<TBehavior, TLockPolicy>::shared(const handle_type &handle) noexcept
        : base_type{handle}
    {
    ...
    shared<TBehavior, TLockPolicy>::shared(shared &&rhs) noexcept
        : base_type{rhs._handle}, _ref_counter{
                                      std::move(rhs.access_ref_counter())}
    {
        ...
        noexcept(is_nothrow_deinit{})
    {
        ...
        // `nullptr`.
        access_ref_counter().lose_ownership([this] { base_type::deinit(); });
    ...
    template <typename TBehavior, typename TLockPolicy>
    void shared<TBehavior, TLockPolicy>::reset(const handle_type &handle) // .
        noexcept(is_nothrow_deinit{})
    {
